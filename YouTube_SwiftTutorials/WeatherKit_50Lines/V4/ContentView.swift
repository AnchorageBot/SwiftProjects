...
//  Refactored by Claude on 3/29/24
//
        ...
        if let weather = weather {
            ...
            VStack {
                ...
                Image(systemName: weather.currentWeather.symbolName)
                    .font(.largeTitle)
                    .padding(.bottom, 50) // Add significant padding below the weather image
        ...
        VStack {
            GeometryReader { geometry in
                HStack(alignment: .bottom, spacing: 20) {
                    ...
                    ForEach(Array(forecasts.enumerated()), id: \.element.date) { index, forecast in
                        VStack {
                            ...
                            Text(forecast.date.weekdayInitial).font(.caption)
            ...
            .frame(height: 200) // Reduce the height of the graph
            
            // Add a label for the graph
            Text("High Temperatures").font(.subheadline)
        }
    }
}
